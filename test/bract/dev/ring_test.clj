...
(ns bract.dev.ring-test
  (:require
    [clojure.test :refer [deftest is testing]]
    [bract.dev.ring :as bd-ring]))
...
(deftest test-log-request
  (doseq [request [{:request-method :get  :uri "/items"}
    ...
    (bd-ring/log-request request)))


(deftest test-log-response
  (doseq [[request response] [[{:request-method :get  :uri "/items"} {:status 200 :headers {"Content-Type" "application/json"}
                                                                      :body "[{\"name\": \"O Ring\", \"unit\": \"each\"}]"}]
                              [{:request-method :post :uri "/items"} {:status 201 :headers {"Location" "http://myapp.com/item/12"}}]
                              [{:request-method :put  :uri "/item/1"} {:status 401 :headers {"Content-Type" "text/plain"}
                                                                       :body "Not authorized to update item"}]
                              ;; non-map response
                              [{:request-method :get  :uri "/foo"}    "Bad request"]
                              [{:request-method :post :uri "/bar"}    39993]]]
    (bd-ring/log-response request response (* ^long (rand-int 50) ^double (rand)))))
...
(deftest test-log-zexception
  (doseq [[request exception] [[{:request-method :get    :uri "/items"}      (Exception. "test")]
    ...
    (bd-ring/log-exception request exception (* ^long (rand-int 50) ^double (rand)))))
